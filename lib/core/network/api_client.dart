...
class ApiClient {
  ...
  Future<http.Response> postFormDataWithFile(String path, {required Map<String, String> fields, Map<String, String>? filePaths, bool auth = false}) async {
    ...
    if (filePaths != null) {
      for (final entry in filePaths.entries) {
        ...
        if (await file.exists()) {
          ...
          request.files.add(http.MultipartFile.fromBytes(entry.key, fileContent, filename: file.path.split('/').last));
        }
      }
    }
      ...
      if (refreshed) {
        ...
        if (filePaths != null) {
          for (final entry in filePaths.entries) {
            ...
            if (await file.exists()) {
              final fileContent = await file.readAsBytes();
              retriedRequest.files.add(http.MultipartFile.fromBytes(entry.key, fileContent, filename: file.path.split('/').last));
            }
          }
        }
